 ...
 * Fitur Notifikasi In-App — Role Karyawan
 * E-Outsourcing PT Ecogreen Oleochemicals
 */

import { apiFetch, toast, _escapeHtml as esc } from './_utils.js';
...
function init() {
    console.log('[Notifikasi Karyawan] Init function called');
    
    // Selector tombol lonceng di topbar
    const btnNotif = document.getElementById('btn-notif');

    console.log('[Notifikasi Karyawan] Button element:', btnNotif);

    if (!btnNotif) {
        console.warn('[Notifikasi Karyawan] Tombol notifikasi tidak ditemukan');
        return; // guard: jangan jalankan jika elemen tidak ada
    }

    console.log('[Notifikasi Karyawan] Init berhasil, tombol notifikasi ditemukan');

    // Set URL "Lihat semua" berdasarkan role (untuk Karyawan)
    const seeAllLink = document.getElementById('notif-see-all-link');
    if (seeAllLink) {
        seeAllLink.href = '/karyawan/notifikasi';
    }
    ...
    if (backdrop) {
        ...
        console.warn('[Notifikasi Karyawan] Backdrop tidak ditemukan');
    }
    ...
    if (btnTutup) {
        ...
        console.warn('[Notifikasi Karyawan] Tombol tutup panel tidak ditemukan');
    }
    ...
    if (btnTandaiSemua) {
        ...
        console.warn('[Notifikasi Karyawan] Tombol tandai semua tidak ditemukan');
    }
    ...
    if (window.location.pathname === '/karyawan/notifikasi') {
        initHalamanPenuh();
    }
}
...
async function fetchBadgeCount() {
    try {
        ...
        if (res.status && res.data) {
            updateBadge(res.data.jumlah_belum_dibaca || 0);
        }
    } catch (err) {
        console.error('[Notifikasi Karyawan] Gagal fetch badge count:', err);
    }
}
...
function openPanel() {
    ...
    if (!overlay) {
        console.error('[Notifikasi Karyawan] Panel overlay tidak ditemukan. Pastikan markup panel sudah ditambahkan di layouts/karyawan.blade.php');
        return;
    }

    console.log('[Notifikasi Karyawan] Membuka panel overlay');
    
    overlay.classList.add('k-notif--open');
...
function closePanel() {
    ...
    overlay.classList.remove('k-notif--open');
...
async function fetchPanelNotifikasi() {
    ...
    if (!listContainer) {
        console.warn('[Notifikasi Karyawan] Element #notif-panel-list tidak ditemukan');
        return;
    }
    ...
    try {
        ...
        console.error('[Notifikasi Karyawan] Gagal fetch panel notifikasi:', err);
...
function renderPanelItems(items) {
    ...
    listContainer.innerHTML = items.map(item => {
        ...
        return `
            <div class="k-notif-item ${isUnread ? 'k-notif-item--unread' : ''}"
                 data-id="${item.id_notifikasi}"
                 data-url="${esc(url)}"
                 role="listitem">
                <div class="k-notif-icon-wrap">
                    ${ikon}
                </div>
                <div class="k-notif-content">
                    <div class="k-notif-title">${esc(item.judul)}</div>
                    <div class="k-notif-time">${esc(waktu)}</div>
                </div>
                ${isUnread ? '<div class="k-notif-unread-dot"></div>' : ''}
            </div>
        `;
    }).join('');

    // Event listener untuk klik item
    listContainer.querySelectorAll('.k-notif-item').forEach(item => {
...
function renderSkeletonPanel() {
    return Array(3).fill(0).map(() => `
        <div class="k-notif-item" style="pointer-events:none;">
            <div class="k-skel k-skel--block" style="width:36px;height:36px;flex-shrink:0;border-radius:var(--radius-md);"></div>
            <div style="flex:1;">
                <div class="k-skel k-skel--text" style="width:80%;height:14px;margin-bottom:6px;"></div>
                <div class="k-skel k-skel--text" style="width:50%;height:12px;"></div>
            </div>
        </div>
    `).join('');
}
...
async function tandaiBaca(id) {
    try {
        ...
        console.error('[Notifikasi Karyawan] Gagal tandai baca:', err);
    }
}
...
async function tandaiSemuaBaca() {
    ...
    try {
        ...
        console.error('[Notifikasi Karyawan] Gagal tandai semua baca:', err);
...
function initHalamanPenuh() {
    ...
    document.querySelectorAll('.k-notif-page-tab').forEach(tab => {
        tab.addEventListener('click', function() {
            // Update active state
            document.querySelectorAll('.k-notif-page-tab').forEach(t => t.classList.remove('k-notif-page-tab--active'));
            this.classList.add('k-notif-page-tab--active');
    ...
    if (btnTandaiSemuaHalaman) {
        btnTandaiSemuaHalaman.addEventListener('click', async function() {
            ...
            try {
                ...
                console.error('[Notifikasi Karyawan] Gagal tandai semua:', err);
                toast('Gagal menandai semua notifikasi', 'error');
            } finally {
                this.disabled = false;
            }
...
async function fetchHalamanPenuh(page = 1, filter = '') {
    ...
    listContainer.innerHTML = Array(5).fill(0).map(() => `
        <div class="k-notif-item" style="display:flex;gap:12px;padding:var(--space-3) var(--space-4);">
            <div class="k-skel k-skel--block" style="width:36px;height:36px;flex-shrink:0;border-radius:var(--radius-md);"></div>
            <div style="flex:1;">
                <div class="k-skel k-skel--text" style="width:80%;height:14px;margin-bottom:6px;"></div>
                <div class="k-skel k-skel--text" style="width:50%;height:12px;"></div>
    ...
    listContainer.innerHTML = data.data.map(item => {
        ...
        return `
            <div class="k-notif-item ${isUnread ? 'k-notif-item--unread' : ''}"
                 ...
                 data-url="${esc(url)}"
                 role="listitem">
                <div class="k-notif-icon-wrap" style="background:${getIkonColor(item.jenis, true)};">
                    ${ikon}
                </div>
                <div style="flex:1;min-width:0;">
                    <div style="font-size:14px;font-weight:500;color:var(--text-primary);margin-bottom:4px;">${esc(item.judul)}</div>
                    <div style="font-size:12px;color:var(--text-secondary);">${esc(item.isi || '')}</div>
                    <div style="font-size:11px;color:var(--text-muted);margin-top:6px;">${esc(waktu)}</div>
                </div>
                ${isUnread ? '<div class="k-notif-unread-dot"></div>' : ''}
            </div>
        `;
...
function renderPaginasiHalaman(meta) {
    ...
    container.innerHTML = `
        <div style="display:flex;align-items:center;justify-content:space-between;gap:16px;padding:var(--space-3) var(--space-4);">
            <div style="font-size:13px;color:var(--text-secondary);">
                Halaman ${currentPage} dari ${lastPage}
            </div>
            <div style="display:flex;align-items:center;gap:6px;">
                <button class="k-btn k-btn--outline k-btn--sm" ${currentPage <= 1 ? 'disabled' : ''} data-page="${currentPage - 1}">
                    Sebelumnya
                </button>
                <button class="k-btn k-btn--outline k-btn--sm" ${currentPage >= lastPage ? 'disabled' : ''} data-page="${currentPage + 1}">
                    Berikutnya
                </button>
            </div>
        </div>
...
// ═══════════════════════════════════════════════════════════════════════════
//  HELPERS — Karyawan specific mappings
// ═══════════════════════════════════════════════════════════════════════════
...
function getNotifikasiUrl(jenis, id_referensi) {
    ...
    return map[jenis] || '/karyawan/dashboard';
}

function getIkonSvg(jenis) {
    const svgMap = {
        'absensi': `<svg style="width:20px;height:20px;color:var(--eco-600);" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
        ...
        'lembur': `<svg style="width:20px;height:20px;color:#7c3aed;" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" d="M12 8v4l3 3m6-3a9 9 0 1 1-18 0 9 9 0 0 1 18 0z"/>
        </svg>`,
        'izin': `<svg style="width:20px;height:20px;color:var(--eco-500);" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" d="M9 12h6m-6 4h6m2 5H7a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h5.586a1 1 0 0 1 .707.293l5.414 5.414a1 1 0 0 1 .293.707V19a2 2 0 0 1-2 2z"/>
        </svg>`,
        'planning': `<svg style="width:20px;height:20px;color:var(--eco-700);" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 0 0 2-2V7a2 2 0 0 0-2-2H5a2 2 0 0 0-2 2v12a2 2 0 0 0 2 2z"/>
        </svg>`,
        'sistem': `<svg style="width:20px;height:20px;color:var(--text-muted);" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 1 1-18 0 9 9 0 0 1 18 0z"/>
        </svg>`,
    };

    return svgMap[jenis] || `<svg style="width:20px;height:20px;color:var(--text-muted);" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
...
function getIkonColor(jenis, isBackground) {
    const colorMap = {
        'absensi' : { bg: 'var(--eco-50)', fg: 'var(--eco-600)' },
        'lembur'  : { bg: '#f5f3ff', fg: '#7c3aed' },
        'izin'    : { bg: 'var(--eco-50)', fg: 'var(--eco-500)' },
        'planning': { bg: 'var(--eco-50)', fg: 'var(--eco-700)' },
        'sistem'  : { bg: 'var(--surface-bg)', fg: 'var(--text-muted)' },
    };
    const color = colorMap[jenis] || { bg: 'var(--surface-bg)', fg: 'var(--text-muted)' };
