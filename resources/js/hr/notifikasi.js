 ...
 * Fitur Notifikasi In-App — Role HR
...
function init() {
    console.log('[Notifikasi HR] Init function called');
    ...
    console.log('[Notifikasi HR] Button element:', btnNotif);

    if (!btnNotif) {
        console.warn('[Notifikasi HR] Tombol notifikasi tidak ditemukan');
        return; // guard: jangan jalankan jika elemen tidak ada
    }

    console.log('[Notifikasi HR] Init berhasil, tombol notifikasi ditemukan');
    
    // Debug: cek panel overlay ada atau tidak
    const overlay = document.getElementById('notif-panel-overlay');
    console.log('[Notifikasi HR] Panel overlay element:', overlay);

    // Set URL "Lihat semua" berdasarkan role (untuk HR)
    const seeAllLink = document.getElementById('notif-see-all-link');
    if (seeAllLink) {
        seeAllLink.href = '/hr/notifikasi';
    }
    ...
    if (backdrop) {
        ...
        console.warn('[Notifikasi HR] Backdrop tidak ditemukan');
    }
    ...
    if (btnTutup) {
        ...
        console.warn('[Notifikasi HR] Tombol tutup panel tidak ditemukan');
    }
    ...
    if (btnTandaiSemua) {
        ...
        console.warn('[Notifikasi HR] Tombol tandai semua tidak ditemukan');
    }
    ...
    if (window.location.pathname === '/hr/notifikasi') {
        initHalamanPenuh();
    }
}
...
async function fetchBadgeCount() {
    try {
        ...
        console.error('[Notifikasi HR] Gagal fetch badge count:', err);
    }
}
...
function openPanel() {
    const overlay = document.getElementById('notif-panel-overlay');
    if (!overlay) {
        console.error('[Notifikasi HR] Panel overlay tidak ditemukan. Pastikan kondisi @if di layouts/app.blade.php sudah benar dan role adalah hr');
        return;
    }

    console.log('[Notifikasi HR] Membuka panel overlay');
...
async function fetchPanelNotifikasi() {
    ...
    if (!listContainer) {
        console.warn('[Notifikasi HR] Element #notif-panel-list tidak ditemukan');
        return;
    }
    ...
    try {
        ...
        console.error('[Notifikasi HR] Gagal fetch panel notifikasi:', err);
...
async function tandaiBaca(id) {
    try {
        ...
        console.error('[Notifikasi HR] Gagal tandai baca:', err);
    }
}
...
async function tandaiSemuaBaca() {
    ...
    try {
        ...
        console.error('[Notifikasi HR] Gagal tandai semua baca:', err);
    ...
    if (btnTandaiSemuaHalaman) {
        btnTandaiSemuaHalaman.addEventListener('click', async function() {
            ...
            try {
                ...
                console.error('[Notifikasi HR] Gagal tandai semua:', err);
                toast('Gagal menandai semua notifikasi', 'error');
            } finally {
                this.disabled = false;
            }
...
async function fetchHalamanPenuh(page = 1, filter = '') {
    ...
    try {
        ...
        console.error('[Notifikasi HR] Gagal fetch halaman penuh:', err);
...
function getIkonColor(jenis, isBackground) {
    const colorMap = {
        'izin'    : { bg: '#f0fdf4', fg: '#16a34a' },
        'absensi' : { bg: '#f0fdf4', fg: '#22c55e' },
        'lembur'  : { bg: '#f5f3ff', fg: '#7c3aed' },
        'planning': { bg: '#f0fdf4', fg: '#15803d' },
