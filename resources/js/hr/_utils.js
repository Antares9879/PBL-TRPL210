/**
 * resources/js/hr/_utils.js
 * Shared utilities untuk semua halaman HR.
 *
 * Pola identik dengan user-departemen/_utils.js, disesuaikan:
 *   - Warna accent: green (bukan teal)
 */
...
export function badgeDokumen(status) {
    const map = {
        lengkap:        `<span class="badge badge--success">Lengkap</span>`,
        tidak_lengkap:  `<span class="badge badge--danger">Tidak Lengkap</span>`,
...
export function injectModalStyles() {
    if (document.getElementById('hr-modal-styles')) return;
    const s = document.createElement('style');
    s.id = 'hr-modal-styles';
    s.textContent = `
        ...
        .skel{background:linear-gradient(90deg,#f1f5f9 25%,#e2e8f0 50%,#f1f5f9 75%);
            background-size:200% 100%;animation:hrShimmer 1.5s ease infinite;}
        @keyframes hrShimmer{0%{background-position:200% 0}100%{background-position:-200% 0}}
    `;
