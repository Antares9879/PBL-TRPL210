 ...
 * Fitur Notifikasi In-App — Role Admin Outsource
 * E-Outsourcing PT Ecogreen Oleochemicals
 */

import { apiFetch, toast, esc } from './_utils.js';
...
function init() {
    ...
    if (window.location.pathname === '/admin/notifikasi') {
        initHalamanPenuh();
    }
}
...
async function fetchBadgeCount() {
    try {
        ...
        console.error('[Notifikasi Admin] Gagal fetch badge count:', err);
    }
}
...
function updateBadge(jumlah) {
    ...
    if (jumlah > 0) {
        if (dot) {
            ...
            dot.setAttribute('data-count', jumlah);
        }
        if (sidebarBadge) {
            sidebarBadge.style.display = 'inline-flex';
            sidebarBadge.textContent = jumlah > 99 ? '99+' : jumlah;
        }
    } else {
...
function openPanel() {
    ...
    if (!overlay) {
        console.error('[Notifikasi Admin] Panel overlay tidak ditemukan. Pastikan markup panel sudah ditambahkan di layouts/admin.blade.php');
        return;
    }

    overlay.classList.add('a-notif--open');
...
function closePanel() {
    ...
    overlay.classList.remove('a-notif--open');
...
async function fetchPanelNotifikasi() {
    ...
    try {
        ...
        console.error('[Notifikasi Admin] Gagal fetch panel notifikasi:', err);
...
function renderPanelItems(items) {
    ...
    listContainer.innerHTML = items.map(item => {
        ...
        return `
            <div class="a-notif-item ${isUnread ? 'a-notif-item--unread' : ''}"
                 data-id="${item.id_notifikasi}"
                 data-url="${esc(url)}"
                 role="listitem">
                <div class="a-notif-icon-wrap">
                    ${ikon}
                </div>
                <div class="a-notif-content">
                    <div class="a-notif-title">${esc(item.judul)}</div>
                    <div class="a-notif-time">${esc(waktu)}</div>
                </div>
                ${isUnread ? '<div class="a-notif-unread-dot"></div>' : ''}
            </div>
        `;
    }).join('');

    // Event listener untuk klik item
    listContainer.querySelectorAll('.a-notif-item').forEach(item => {
...
function renderSkeletonPanel() {
    return Array(3).fill(0).map(() => `
        <div class="a-notif-item" style="pointer-events:none;">
            <div class="skel" style="width:36px;height:36px;flex-shrink:0;border-radius:var(--radius-md);"></div>
            <div style="flex:1;">
                <div class="skel" style="width:80%;height:14px;margin-bottom:6px;border-radius:4px;"></div>
                <div class="skel" style="width:50%;height:12px;border-radius:4px;"></div>
            </div>
        </div>
    `).join('');
}
...
async function tandaiBaca(id) {
    try {
        ...
        console.error('[Notifikasi Admin] Gagal tandai baca:', err);
    }
}
...
async function tandaiSemuaBaca() {
    ...
    try {
        ...
        console.error('[Notifikasi Admin] Gagal tandai semua baca:', err);
...
function initHalamanPenuh() {
    ...
    document.querySelectorAll('.a-notif-page-tab').forEach(tab => {
        tab.addEventListener('click', function() {
            // Update active state
            document.querySelectorAll('.a-notif-page-tab').forEach(t => t.classList.remove('a-notif-page-tab--active'));
            this.classList.add('a-notif-page-tab--active');
    ...
    if (btnTandaiSemuaHalaman) {
        btnTandaiSemuaHalaman.addEventListener('click', async function() {
            ...
            try {
                ...
                console.error('[Notifikasi Admin] Gagal tandai semua:', err);
                toast('Gagal menandai semua notifikasi', 'error');
            } finally {
                this.disabled = false;
            }
...
async function fetchHalamanPenuh(page = 1, filter = '') {
    ...
    listContainer.innerHTML = Array(5).fill(0).map(() => `
        <div class="a-notif-item" style="display:flex;gap:12px;padding:var(--space-3, 12px) var(--space-4, 16px);">
            <div class="skel" style="width:36px;height:36px;flex-shrink:0;border-radius:8px;"></div>
            <div style="flex:1;">
                <div class="skel" style="width:80%;height:14px;margin-bottom:6px;border-radius:4px;"></div>
                <div class="skel" style="width:50%;height:12px;border-radius:4px;"></div>
            </div>
    ...
    try {
        ...
        console.error('[Notifikasi Admin] Gagal fetch halaman penuh:', err);
    ...
    listContainer.innerHTML = data.data.map(item => {
        ...
        return `
            <div class="a-notif-item ${isUnread ? 'a-notif-item--unread' : ''}"
                 ...
                 data-url="${esc(url)}"
                 role="listitem">
                <div class="a-notif-icon-wrap" style="background:${getIkonColor(item.jenis, true)};">
                    ${ikon}
                </div>
...
function renderPaginasiHalaman(meta) {
    ...
    container.innerHTML = `
        <div style="display:flex;align-items:center;justify-content:space-between;gap:16px;padding:var(--space-3, 12px) var(--space-4, 16px);">
            <div style="font-size:13px;color:#64748b;">
                Halaman ${currentPage} dari ${lastPage}
            </div>
            <div style="display:flex;align-items:center;gap:6px;">
                <button class="a-btn a-btn--outline a-btn--sm" ${currentPage <= 1 ? 'disabled' : ''} data-page="${currentPage - 1}">
                    Sebelumnya
                </button>
                <button class="a-btn a-btn--outline a-btn--sm" ${currentPage >= lastPage ? 'disabled' : ''} data-page="${currentPage + 1}">
                    Berikutnya
                </button>
            </div>
        </div>
...
// ═══════════════════════════════════════════════════════════════════════════
//  HELPERS — Admin Outsource specific mappings
// ═══════════════════════════════════════════════════════════════════════════
...
function getNotifikasiUrl(jenis, id_referensi) {
    ...
    return map[jenis] || '/admin/dashboard';
}

function getIkonSvg(jenis) {
    const svgMap = {
        'absensi': `<svg style="width:20px;height:20px;color:var(--amber-600, #d97706);" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" d="M9 12l2 2 4-4m6 2a9 9 0 1 1-18 0 9 9 0 0 1 18 0z"/>
        </svg>`,
        'izin': `<svg style="width:20px;height:20px;color:var(--amber-500, #f59e0b);" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" d="M9 12h6m-6 4h6m2 5H7a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h5.586a1 1 0 0 1 .707.293l5.414 5.414a1 1 0 0 1 .293.707V19a2 2 0 0 1-2 2z"/>
        </svg>`,
        'planning': `<svg style="width:20px;height:20px;color:var(--amber-700, #b45309);" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
        ...
        'sistem': `<svg style="width:20px;height:20px;color:#94a3b8;" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 1 1-18 0 9 9 0 0 1 18 0z"/>
        </svg>`,
    };

    return svgMap[jenis] || `<svg style="width:20px;height:20px;color:#94a3b8;" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
...
function getIkonColor(jenis, isBackground) {
    const colorMap = {
        'absensi' : { bg: 'var(--amber-50, #fffbeb)', fg: 'var(--amber-600, #d97706)' },
        'izin'    : { bg: 'var(--amber-50, #fffbeb)', fg: 'var(--amber-500, #f59e0b)' },
        'planning': { bg: 'var(--amber-50, #fffbeb)', fg: 'var(--amber-700, #b45309)' },
        'lembur'  : { bg: '#f5f3ff', fg: '#7c3aed' },
        'sistem'  : { bg: '#f8fafc', fg: '#94a3b8' },
    };
    const color = colorMap[jenis] || { bg: '#f8fafc', fg: '#94a3b8' };
